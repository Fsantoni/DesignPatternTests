...
\renewcommand\bibname{Sitography}
...
In software engineering, a design pattern is a general reusable template to solve a commonly occurring problem within a given context.
...
\newline
We have thus represented the field's interaction with the class methods through a data flow graph in which the granularity was set such that basic blocks represent functions.
...
We then decided to test the interaction between the variable and the methods following the\textit{ all-uses} criterion, in we fact deemed the \textit{all-def} criterion not useful to test this pattern because we value mainly the transmission of the right value of the field and thus all its \textit{uses}.   
...
\chapter*{Conclusions}
We identified a collection of design patterns; for each pattern we produced an implementation 

1. identificare una collezione di 6-9 design patterns interessanti da testare
1.1 come ipotesi iniziale:
observer
adapter (class e object)
decorator
proxy
visitor
state
...
2. per ciascun pattern
2.1 realizzare un'implementazione
2.2 sviluppare una test suite ragionata, riferibile a un qualche criterio di copertura, ispirata a criteri di effciienza, basati su un ragionato fault model
2.3 realizare i tests usando JUnit (e possibilmente Mockito)


3. l'esame poi consiste nel discutere la realizzazioen, presnetare il modo con cui funzioanno JUnit e Mockito, discutere il razionale assunto per i diversi design patterns considerati ...

In allegato vi passo le poche note della lezione di MVT sul tema, e anche una trascrizioen delle note del corso realizzata da una vostra collega.


\begin{thebibliography}{9}

\bibitem{1} JUnit \url{http://junit.org/junit4/}
\bibitem{2} Mockito \url{http://mockito.org/}
\bibitem{3} EclEmma \url{http://www.eclemma.org/}
