...
We generated a test suite capable of testing all the \textit{all-uses} paths:
\begin{enumerate}
	\item Subject() setState()[notify()]	<- controllo che con lista vuota non lanci errori
	\item Subject() detach()				 <-1) In questo caso il driver fa le veci dell'"Object", ne chiamo il metodo e lo "lascio fare", identico anche in integration test 
	2) sto testando detach prima di attach: controllo non dia errori	
	\item Subject() attach() attach() attach() detach() detach()  (+setState()[notify()]) x4 attach attach E attach detach E detach detach E detach notify
	\item Subject() attach() attach() detach() detach() attach() (+setState()[notify()]) x3 subject attach E detach attach E attach notify 
