...
24 Agosto 2016 }}%inserire l'anno accademico a cui si ÃÂ¨ iscritti
\end{center}
\end{titlepage}
...
\begin{itemize}
	...
	\item \textbf{Adapter}: inherits from both Adaptee and Target, adapts the legacy methods to the desired interface  
\end{itemize}
...
\begin{itemize}
	\item \textbf{Adaptee}: legacy class with specific methods and fields
	\item \textbf{AdapteeOpposite}: inherits form the legacy class, overrides the getter method
	\item \textbf{Target}: desired interface
	\item \textbf{Adapter}: inherits from Target, adapts the legacy methods to the desired interface through delegation
\end{itemize}
...
\begin{figure}[!h]
	...
	\caption{Data Flow Graph: \textit{adaptee}}
	\label{OAdataflow}
\end{figure}
...
\begin{itemize}
	...
	\item ConcreteDecorator ConcreteComponent
...
\begin{itemize}
	...
	\item the composite-specific methods produce unexpected results(either do not throw exceptions when applied on a Leaf or produce wrong results when applied on a Composite)
\end{itemize}
...
The problem of testing thus expands in two different orthogonal dimensions: data flow, the series of calls that modify or use the variable \textit{vector},  and topology, the different hierarchies of objects with respect to the method \textit{operation}.


We decided to explore both of them separately in specific focused tests.

\paragraph{Data Flow: field \textit{vector}}
We have represented the field's interaction with the class methods through a data flow graph in which the granularity was set such that basic blocks represent functions.
...
\begin{figure}[!h]
	...
	\caption{Data Flow Graph: \textit{vector}}
	\label{Codataflow}
\end{figure}
...
The field was tested in a fixed hierarchy formed with a Composite element containing another Composite, which contained a Leaf.  
...
To be noted though is that the fixed hierarchy actually changes during the execution of the tests due to the application of methods like \textit{add() }and \textit{remove()}
...
\paragraph{Data Flow: field \textit{vector}}
We generated a test suite capable of testing most \textit{all-uses} paths:
\begin{itemize}
...
The cases where \textit{remove()} was called before \textit{add()} are not considered, we pass the responsibility of not calling the methods in such order to the Client.
...
\subparagraph{Data Flow Graph}
Both Client and followed Subjects interacts with the ConcreteObserver methods.
...
JUnit is an open source unit (and integration) testing framework for the Java programming language. JUnit has been important in the development of test-driven development and is one of a family of unit testing frameworks which is collectively known as xUnit that originated with SUnit.
