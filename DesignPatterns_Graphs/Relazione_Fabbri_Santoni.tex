...
\subsubsection{Tests}
We generated a test suite capable to test all the \textit{all-uses} paths:
\begin{enumerate}
	\item Adapter() GetInt() 
...
The special case of
\begin{enumerate}
		\item \item Adapter() setBool() getbool() 
		
\end{enumerate}	
is not tested because all the methods are directly inherited from the legacy (and thus supposedly already tested) class and no side-effects, which could have invalidated some invariants, are introduced in the adapter.
...
The code coverage measure obtained from EclEmma Java plugin is: 100%.
...
The composition adds the possibility of dynamically switching the adapted legacy class.


\subsubsection{Class Diagram}
\begin{figure}[!h]
	\centering
	\includegraphics[width=0.3\textwidth]{./Adapter/Object/ClassDiagram.png}
...
\begin{itemize}
	\item \textbf{Adaptee}: legacy class with specific methods and fields
	\item \textbf{Target}: desired interface
	\item \textbf{ObjectAdapter}: inheriting from Target, adapts the legacy methods to the desired interface through delegation. 
\end{itemize}
