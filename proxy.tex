...
It provides a representation of an object difficult to access or which requires an important time to access.
...
\begin{itemize}
	\item \textbf{Proxy}: maintains a reference to access the RealSubject, implements a identical interface to that of RealSubject, so that it may replace it, check the access to RealSubject, being responsible for its instantiation and management of references.
	\item \textbf{SubjectInterface}: defines the interface used by the Client that is implemented by the Proxy and the RealSubject.
	\item \textbf{RealSubject}: defines the real object of which the proxy will be responsible to surrogate.
\end{itemize}

\subsubsection{Fault Model}

The pattern focuses on treating uniformly individual and compound objects. Grave error is found in the following situation:  
\begin{itemize}
	\item access to RealSubject or his creation leads to errors	
\end{itemize}

\subsection{Testing}

We are concerned that the methods always work, in particular, this criteria is obtain with an "all use" method on RealSubject field.
\newline
...
\end{enumerate}
Since in this pattern the tested class's methods (Proxy's) interacted with external classes (SubjectInterface) we  differentiated between Unit and Integration tests.

We utilized Mockito's \textit{mock} function to isolate errors with an origin in external classes from interfering with the Adapter class's code.

\subsubsection{Code Coverage}
The code coverage measure obtained from EclEmma Java plugin is: \% 
\begin{itemize}
	\item Proxy: \% (UnitTest \%)
